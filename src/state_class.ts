...
import { TIStylesBoolean, TStyleArray, TIStylesNumber, TIStylesString, mxCell, GFP, TPropertieKey, TStyleKey } from '../types';
...
export default class State {
  ...
  static getDefaultValueStyles(): TIStylesString {
    return {
      fillColor: null,
      strokeColor: null,
      fontColor: null,
  ...
  static getDefaultFlagStyles(): TIStylesBoolean {
    return {
      fillColor: false,
      strokeColor: false,
      fontColor: false,
   ...
   * @memberof State
   */
  getCellProp(prop: TPropertieKey) {
    if (prop === 'id') {
      return this.cellId;
    }
    if (prop === 'value') {
   ...
   * @memberof State
   */
  unsetLevelStyle(style: TStyleKey) {
    this.level[style] = -1;
  }
