...
EditorUi = function EditorUi(editor, container, lightbox) {
  ...
  graph.lightbox = lightbox; // Faster scrollwheel zoom is possible with CSS transforms
  ...
  this.hoverIcons = this.createHoverIcons(); // Adds tooltip when mouse is over scrollbars to show space-drag panning option
  ...
  var connectStyles = ['shape', 'edgeStyle', 'curved', 'rounded', 'elbow', 'comic', 'jumpStyle', 'jumpSize']; // Note: Everything that is not in styles is ignored (styles is augmented below)
  ...
  var valueStyles = ['fontFamily', 'fontSize', 'fontColor']; // Keys that always update the current edge style regardless of selection
    ...
    try {
      ...
      if (asText) {
        ...
        for (var i = 0; i < cells.length; i++) {
          var cell = cells[i]; // Removes styles defined in the cell style from the styles to be applied
...
function ChangePageSetup(ui, color, image, format) {
  ...
  this.format = format;
  this.previousFormat = format; // Needed since null are valid values for color and image
