import { GFGlobal, GFVariables } from '../src/globals_class';
...
describe.skip('GFGlobal', () => {
  describe('utils ID', () => {
    test('Should not Nan', () => {
      expect(GFGlobal.utils.uniqueID()).not.toBeNaN();
    });
    test('Should should be uniq', () => {
      expect(GFGlobal.utils.uniqueID()).not.toBe(GFGlobal.utils.uniqueID());
    });
  });

  describe('Encode/Decode', () => {
    test('Should Decode', () => {
      expect(GFGlobal.utils.decode(compGraph, true, true, true)).toBe(xmlGraph);
    });
    test('Should Encode', () => {
      expect(GFGlobal.utils.encode(xmlGraph, true, true, true)).toBe(compGraph);
    });
  });

  describe('Variables', () => {
    const variable = GFGlobal.createLocalVars();
    const key = '_value';
    const value = 12345;
