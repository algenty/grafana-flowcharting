import { _GF, GFVariables } from '../src/globals_class';
...
describe('_GF', () => {
  describe('utils ID', () => {
    test('Should not Nan', () => {
      expect(_GF.utils.uniqueID()).not.toBeNaN();
    });
    test('Should should be uniq', () => {
      expect(_GF.utils.uniqueID()).not.toBe(_GF.utils.uniqueID());
    });
  });

  describe('Encode/Decode', () => {
    test('Should Decode', () => {
      expect(_GF.utils.decode(compGraph, true, true, true)).toBe(xmlGraph);
    });
    test('Should Encode', () => {
      expect(_GF.utils.encode(xmlGraph, true, true, true)).toBe(compGraph);
    });
  });

  describe('Secure Eval', () => {
    test('Math', () => {
      expect(_GF.utils.evalIt('Math.random()*101')).toBeGreaterThan(0);
      expect(_GF.utils.evalIt('Math.random()*101')).toBeLessThanOrEqual(100);
    });
    test('Simple operation', () => {
      expect(_GF.utils.evalIt('2+2')).toBe(4);
    });
  });

  describe('Variables', () => {
    const variable = _GF.createLocalVars();
    const key = '_value';
    const value = 12345;
