import Flowchart from '../src/flowchart_class';
...
describe('Create Flowchart', () => {
  const data = {};
  const fc = new Flowchart('main', xmlGraph, container, ctrl, data);
  fc.setXml(xmlGraph);
  test('Should be object', () => {
    expect(fc).toBeInstanceOf(Object);
  });
  test('Snapshot', () => {
    expect(fc).toMatchSnapshot('flowchart_initial');
  });
});

describe('Init Flowchart', () => {
  const data = {};
  const fc = new Flowchart('main', xmlGraph, container, ctrl, data);
  fc.setXml(xmlGraph);
  test('Prettify', () => {
    fc.prettify();
    expect(fc.data.xml).toMatchSnapshot('flowchart_prettify');
  });

  test('Minify', () => {
    fc.prettify();
    expect(fc.data.xml).toMatchSnapshot('flowchart_minify');
  });
});
